...
t2_neg:		xor	di,true			; if true already becomes false
		...
		neg	cx
                sbb	dx,0
                mov	[n2_temp], cx		; save to n2_temp
                mov	[n2_temp+2],dx
...
div_loop:      	cmp	dx, [n2_temp+2]       		; while (num2 >= original_num2)
                jg	do_div
                cmp	dx, [n2_temp+2]
                jne	neg_check
                cmp	cx, [n2_temp]
...
neg_check:	pop	ax			; restore negative true/false off the stack
